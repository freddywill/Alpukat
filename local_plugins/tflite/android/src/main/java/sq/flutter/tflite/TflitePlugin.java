...
public void onDetachedFromEngine(@NonNull FlutterPluginBinding binding) {
  ...
  public void onMethodCall(MethodCall call, Result result) {
        if (call.method.equals("loadModel")) {
            ...
            result.error("Invalid method", call.method, "");
        }
    }
  ...
  private String loadModel(HashMap args) throws IOException {
    ...
    if (isAsset) {
      ...
      key = flutterAssets.getAssetFilePathByName(model);
    ...
    if (labels.length() > 0) {
      if (isAsset) {
        String labelAssetKey = flutterAssets.getAssetFilePathByName(labels);
        loadLabels(assetManager, key, interpreter);
      } else {
        loadLabels(null, labels, interpreter);
      }
    }
  ...
  public void initInterpreter(AssetManager assetManager, String modelPath) {
        try {
            MappedByteBuffer modelBuffer = loadModelFile(assetManager, modelPath);
            interpreter = new Interpreter(modelBuffer);
            Log.d("TFLITE_PLUGIN", "Interpreter berhasil diinisialisasi.");
        } catch (IOException e) {
            Log.e("TFLITE_PLUGIN", "Gagal memuat model: " + e.getMessage());
            interpreter = null;
        }
    }

  public void loadModel(AssetManager assetManager, String modelPath, String labelPath) {
        try {
            MappedByteBuffer modelBuffer = loadModelFile(assetManager, modelPath);
            interpreter = new Interpreter(modelBuffer);
            Log.d("TFLITE_PLUGIN", "Interpreter berhasil dibuat");

            loadLabels(assetManager, labelPath, interpreter);
        } catch (IOException e) {
            Log.e("TFLITE_PLUGIN", "Gagal memuat model: " + e.getMessage());
            interpreter = null;
        }
    ...
    // Method untuk memuat file model TensorFlow Lite dari asset
  ...
  private void loadLabels(AssetManager assetManager, String path, Interpreter interpreter) {
    if (interpreter == null) {
        Log.e("TFLITE_PLUGIN", "Interpreter belum diinisialisasi, tidak bisa memuat label");
        return;
    }

    labels = new ArrayList<>();

    try (BufferedReader br = (assetManager != null)
        ...
        // Ambil shape output tensor dari interpreter
        int[] outputShape = interpreter.getOutputTensor(0).shape();  // Contoh: [1, 5]
        int outputSize = outputShape[1];

        // Log jumlah label dan output model
        Log.d("TFLITE_PLUGIN", "Jumlah label yang dimuat: " + labels.size());
        Log.d("TFLITE_PLUGIN", "Output model shape: " + Arrays.toString(outputShape));

        // Cek apakah jumlah label sama dengan output model
        if (labels.size() != outputSize) {
            Log.w("TFLITE_PLUGIN", "Peringatan: jumlah label (" + labels.size() + ") tidak sama dengan output model (" + outputSize + ")");
            // Anda bisa pilih untuk menyesuaikan labels atau tetap lanjut
        }

        // Buat array output berdasarkan output model, bukan label
  ...
  public float[][] runInference(float[][] inputData) {
        if (interpreter == null) {
            Log.e("TFLITE_PLUGIN", "Interpreter belum diinisialisasi.");
            return null;
        }
        if (labelProb == null) {
            Log.e("TFLITE_PLUGIN", "LabelProb belum diinisialisasi, pastikan loadLabels sudah dipanggil.");
            return null;
        }
        interpreter.run(inputData, labelProb);
        return labelProb;
    }


  public List<String> getTopN(float[] probabilities, int topN) {
        if (labels == null) {
            Log.e("TFLITE_PLUGIN", "Labels belum dimuat, tidak bisa mengambil TopN");
            return new ArrayList<>();
        }

        PriorityQueue<Integer> pq = new PriorityQueue<>(topN, new Comparator<Integer>() {
            @Override
            public int compare(Integer o1, Integer o2) {
                return Float.compare(probabilities[o1], probabilities[o2]);
            }
        });
        ...
        for (int i = 0; i < probabilities.length; i++) {
            if (pq.size() < topN) {
                pq.add(i);
            } else if (probabilities[i] > probabilities[pq.peek()]) {
                pq.poll();
                pq.add(i);
            }
        }
        ...
        while (!pq.isEmpty()) {
            int index = pq.poll();
            topLabels.add(labels.get(index) + " : " + probabilities[index]);
        }

        // Urutkan descending berdasarkan probabilitas
        topLabels.sort((a, b) -> {
            float probA = Float.parseFloat(a.substring(a.lastIndexOf(":") + 2));
            float probB = Float.parseFloat(b.substring(b.lastIndexOf(":") + 2));
            return Float.compare(probB, probA);
        });
